...
export default function HistoricoFiatScreen() {
  ...
  const carregarHistorico = async () => {
    ...
    if (data) {
      ...
      const fiat = todos.filter((item: any) => item.moeda === 'Dólar' || item.moeda === 'Euro');
      setHistorico(fiat);
    }
  };
  ...
  const limparHistorico = async () => {
    ...
    if (data) {
      const todos = JSON.parse(data);
      const apenasCripto = todos.filter((item: any) => item.moeda !== 'Dólar' && item.moeda !== 'Euro');
      await AsyncStorage.setItem('historico', JSON.stringify(apenasCripto));
    }
    setHistorico([]);
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>📜 Histórico de Dólar e Euro</Text>
      <FlatList
